...
monCompte(API_ROOT."?controller=users&action=single&id=".$_SESSION['id_membre']);
listerVehicule(API_ROOT."?controller=vehicule&action=index&id_membre=".$_SESSION['id_membre']);
if(!empty($_POST)){
	modifCompte();
}
...
$ch = curl_init(API_ROOT."?controller=users&action=single&id=".$_SESSION['id_membre']);
...
if(!$response) {
 ...
 function deleteVehicule($id_vehicule){
	...
	$ch = curl_init(API_ROOT.'?controller=vehicule&action=single&id='.$id_vehicule);
